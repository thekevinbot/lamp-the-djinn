...
if ! grep -q 'SSH_AUTH_SOCK' "$CONFIG"; then
    ...
    if command -v jq &>/dev/null; then
        jq '.mounts += ["source=${localEnv:SSH_AUTH_SOCK},target=/ssh-agent,type=bind", "source=${localEnv:HOME}/.ssh,target=/home/node/.ssh,type=bind,readonly"] | .containerEnv.SSH_AUTH_SOCK = "/ssh-agent"' "$CONFIG" > "$tmp"
    else
        # Fallback: add mount to existing mounts array and add containerEnv
        sed '/"mounts":/,/\]/ s|\]|, "source=\${localEnv:SSH_AUTH_SOCK},target=/ssh-agent,type=bind", "source=\${localEnv:HOME}/.ssh,target=/home/node/.ssh,type=bind,readonly"]|' "$CONFIG" | \
